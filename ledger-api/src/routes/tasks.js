...
async function taskTeamId(id) {
  const result = await pool.query('SELECT team_id FROM tasks WHERE id = $1', [id]);
...
tasksRouter.patch('/:id', async (req, res, next) => {
  try {
    const { id } = req.params;
    const teamId = await taskTeamId(id);
    if (!teamId) return res.status(404).json({ error: 'task not found' });
    if (!(await assertTeamMember(teamId, req.memberId))) {
      return res.status(403).json({ error: 'not a member of this team' });
    }
